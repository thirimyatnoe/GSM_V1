...
Public Class frm_rpt_SaleGems

    Private ReportDA As SaleGems.ISaleGemsController = Factory.Instance.CreateSaleGemsController
...
#End Region
    ...
    Private Sub frm_rpt_SaleGems_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        cboGemsCategory.Enabled = False
        CboLocation.Enabled = True
        ChkLocation.Checked = True        
        GetCombo()
        CboLocation.SelectedValue = Global_CurrentLocationID
        Me.rptSaleGems.RefreshReport()
    End Sub
    ...
    Private Sub btnPreview_Click_1(sender As Object, e As EventArgs) Handles btnPreview.Click
        ...
        If ChkLocation.Checked Then
            dt = ReportDA.GetSaleGemsItemForRpt(dtpFromDate.Value.Date, dtpToDate.Value.Date, GetFilterString)
        Else
        ...
        End If
        rptSaleGems.Reset()
        rptSaleGems.LocalReport.ReportEmbeddedResource = IIf(radSummary.Checked, "UI.rpt_SaleGems_Summary.rdlc", "UI.rpt_SaleGems_Detail.rdlc")
        rptSaleGems.LocalReport.DataSources.Clear()
        rptSaleGems.LocalReport.DataSources.Add(New ReportDataSource("dsSaleGems_SaleGems", dt))
        ...
        If radDetail.Checked Then
            ...
            dtTotal = ReportDA.GetSaleGemsReportForTotal(dtpFromDate.Value.Date, dtpToDate.Value.Date, GetFilterString)
            ...
            rptSaleGems.LocalReport.SetParameters(TotalAmount)

            NetAmount(0) = New ReportParameter("NetAmount", NetAmt)
            rptSaleGems.LocalReport.SetParameters(NetAmount)

            PaidAmount(0) = New ReportParameter("PaidAmount", PaidAmt)
            rptSaleGems.LocalReport.SetParameters(PaidAmount)
            ...
            rptSaleGems.LocalReport.SetParameters(Selected)
        End If
        Dim G_PToY(0) As ReportParameter
        G_PToY(0) = New ReportParameter("G_PToY", Global_PToY)
        rptSaleGems.LocalReport.SetParameters(G_PToY)
        rptSaleGems.RefreshReport()
    End Sub
    Private Function GetFilterString() As String
        GetFilterString = ""

        If (chkGemsCategory.Checked) Then
            GetFilterString = " And G.GemsCategoryID = '" & cboGemsCategory.SelectedValue & "'"
        End If
        If (ChkLocation.Checked) Then
            GetFilterString += " And S.LocationID = '" & CboLocation.SelectedValue & "'"
        End If
        'If (chkGemsCategory.Checked And chkLocation.Checked) Then
        '    GetFilterString = " And S.LocationID = '" & cboLocation.SelectedValue & "' And SI.GemsCategoryID = '" & cboGemsCategory.SelectedValue & "'"

        'ElseIf (chkGemsCategory.Checked And chkLocation.Checked = False) Then
        '    GetFilterString = " And SI.GemsCategoryID = '" & cboGemsCategory.SelectedValue & "'"

        'ElseIf (chkGemsCategory.Checked = False And chkLocation.Checked) Then
        '    GetFilterString = " And S.LocationID = '" & cboLocation.SelectedValue & "'"
        'End If
    End Function
